...
string MD5::binaryToHexString(const unsigned char* input, size_t length)
{
	string str;
	...
	for (size_t i = 0; i < length; ++i) {
		...
		int a = t / 16;
		int b = t % 16;
		str.append(1, HEX_TABLE[a]);
		str.append(1, HEX_TABLE[b]);
	}
	return str;
}


/* MD5_2 basic transformation. Transforms _state based on block. */
...
/* MD5_2 finalization. Ends an MD5_2 message-_digest operation, writing the
...
void MD5::update(FILE* fp)
{
	...
	while ( !feof(fp) )
	{
		size_t length = fread(buffer,1,BUFFER_SIZE,fp);
		if (length > 0) {
...
/* MD5_2 block update operation. Continues an MD5_2 message-digest
...
const unsigned char* MD5::result()
{
	if (!_finished)
	{
		_finished = true;
...
{
	return binaryToHexString(result(), 16);
}
